...
const MyAssetPlaner = () => {
  ...
  // 추가: 월 고정 예금, 적금, 대출 상환 상태 변수
  const [monthlyFixedDeposit, setMonthlyFixedDeposit] = useState(0);
  const [monthlyFixedInstallmentSavings, setMonthlyFixedInstallmentSavings] = useState(0);
  ...
  // 월 고정 지출 데이터를 가져오는 함수들
  const fetchMonthlyFixedData = async () => {
    try {
      const fetchData = async (url, setter, key) => {
        const response = await fetch(url, { credentials: 'include' });
        const data = await response.json();
        if (response.ok) {
          setter(parseFloat(data[key]) || 0);  // 서버에서 받은 key 값으로 상태 업데이트
        } else {
          console.error(`Failed to fetch data from ${url}:`, data.error);
        }
      };

      await fetchData(`${process.env.REACT_APP_API_URL}/api/my-asset-planer/monthly-fixed-deposit`, setMonthlyFixedDeposit, 'monthlyFixedDeposit');
      await fetchData(`${process.env.REACT_APP_API_URL}/api/my-asset-planer/monthly-fixed-installment-saving`, setMonthlyFixedInstallmentSavings, 'monthlyFixedInstallmentSavings');
      await fetchData(`${process.env.REACT_APP_API_URL}/api/my-asset-planer/total-loan-repayment`, setTotalLoanRepayment, 'totalLoanRepayment');
    } catch (error) {
      console.error('Error fetching monthly fixed data:', error);
    }
  };
  ...
  useEffect(() => {
    if (hasBankAccount === true) {
      ...
      fetchTotalAssetData();
      fetchMonthlyFixedData();  // 월 고정 데이터를 가져오는 함수 호출
    }
  }, [hasBankAccount]);  
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setInvestList([{ date: "", stock: "", action: "", quantity: "" }]); // 입력 필드 초기화
          onClose(); // 모달 닫기
          fetchData(`${process.env.REACT_APP_API_URL}/api/my-asset-planer/stock`, (data) => {
            setSamsungStock(data.samsung || {});
            setAppleStock(data.apple || {});
          ...
          // 판매 관련 에러 처리
          if (data.details) {
            alert(data.details); // 서버에서 전달된 에러 메시지를 알림으로 표시
    ...
    <div className="myassetplaner-dashboard">
      <section className="myassetplaner-summary">
        ...
        <div className="progress-charts">
          ...
          <div className="progress-label">전월 지출 금액</div>
          ...
          <div className="budget">
            ...
            <table className="budget-table">
              ...
              <tbody className="budget-tbody">
                <tr>
                  <td>정기 예금</td>
                  <td>{formatValue(monthlyFixedDeposit)}</td>
                  <td>
                    {isEditingSavings ? (
                      <input
                        type="text"
                ...
                <tr>
                  <td>적금</td>
                  <td>{formatValue(monthlyFixedInstallmentSavings)}</td>
                  <td>
                    {isEditingSavings ? (
                      <input
                        type="text"
            ...
            <table className="loan-table">
              ...
              <tbody className="loan-tbody">
                <tr>
                  ...
                  <td>{formatValue(totalLoanRepayment)}</td>
                  <td>{formatValue(userLoan-totalLoanRepayment)}</td>
                </tr>
              </tbody>
            </table>
            {isEditingLoan && (
